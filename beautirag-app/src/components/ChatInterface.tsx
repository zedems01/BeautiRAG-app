...
interface ChatInterfaceProps {
  selectedModel: string; // Passed down from parent
  apiKey: string;        // Passed down from parent
}
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ selectedModel, apiKey }) => {
  ...
  const handleSendMessage = async () => {
    ...
    const newUserMessage: Message = {
      id: Date.now(), // Using timestamp for potentially better unique ID
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/query/`, {
        ...
        body: JSON.stringify({
          ...
          api_key: apiKey || null, // Send null if empty
        }),
      });
      ...
      const finalBotMessage: Message = {
        id: loadingBotMessage.id, // Reuse ID to replace the loading message
    ...
    <div className="flex flex-col h-[76vh] bg-gray-850 border border-gray-700 p-4 rounded-lg shadow-md text-gray-200">
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-sm lg:max-w-lg px-4 py-2 rounded-xl shadow ${msg.sender === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-gray-700 text-gray-200 rounded-bl-none'} ${msg.isLoading ? 'animate-pulse' : ''}` // Add pulsing animation for loading
              }
              >
                {msg.text}
              </div>
      ...
      <div className="flex items-end border-t border-gray-700 pt-4 mt-2">
        <textarea
          ...
          disabled={isBotLoading} // Disable input while bot is thinking
        />
        <button
          ...
          disabled={!inputValue.trim() || isBotLoading} // Disable button if input is empty or bot is loading
        >
          {isBotLoading ? (
            // Simple spinner
